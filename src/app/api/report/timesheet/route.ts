...
export async function GET(req: NextRequest) {
  ...
  const { data: manager } = await supabase
    .from('employees').select('id, full_name, designation').eq('user_id', user.id).maybeSingle();

  const [{ data: employee }, { data: rawEntries }] = await Promise.all([
    supabase.from('employees')
      .select('id, full_name, designation, email, departments(name)')
  ...
  if (!employee) return new NextResponse('Employee not found or access denied.', { status: 403 });
...
<div class="page">
  ...
  <div class="info-grid">
    <div class="info-col">
      ...
      <div class="info-row"><span class="info-label">Department</span><span class="info-value">${esc(deptName)}</span></div>
